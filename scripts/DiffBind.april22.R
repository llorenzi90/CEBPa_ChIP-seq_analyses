...
samples=read.csv("sample_sheet_csuc.csv")
bsgmm39=readRDS("/scratch/llorenzi/BSgen.mm39.rds")

library(DiffBind)
Mdba=dba(sampleSheet=samples)
...
saveRDS(Mdba.analysis,"Mdba.analysis.apr22.RDS")

peaks=dba.peakset(Mdba.analysis,bRetrieve = T,
                  DataType = DBA_DATA_FRAME)
dir.create("apr22")

write.csv(peaks,"apr22/consensus_peaks.DiffBind.csv",row.names = F)
