...
}else outdir="/scratch/llorenzi/HPC7_public_ChIP_plots/p30UTvsp42UT/"
...
for (peakset in names(erl)) {
  ...
  #if extended_ranges is too long (longer than 5000) then split into chunks    
  chunks=list()
  if(ceiling(length(extended_granges)/5000)>1){
    chunks=ceiling(length(extended_granges)/5000)
