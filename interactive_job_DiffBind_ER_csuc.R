...
##### analysis using input samples as samples###
samples=read.csv("sample_sheet_wiht_input_ER.csv")
...
Mdba.count=dba.count(Mdba.GL)
Mdba.count
# 15 Samples, 37822 sites in matrix:                  ID Factor Condition Treatment Replicate    Reads FRiP1     p30_UT_1_Nov21     ER     p30UT        UT         1 44745780 0.06
# 2     p42_UT_1_Nov21     ER     p42UT        UT         1 42842523 0.04
# 3    p30_LPS_1_Apr22     ER    p30LPS       LPS         1 46928333 0.024     p42_UT_2_Apr22     ER     p42UT        UT         2 53852338 0.025    p30_LPS_2_Apr22     ER    p30LPS       LPS         2 67623252 0.03
# 6    p30_LPS_3_Apr22     ER    p30LPS       LPS         3 61878984 0.02
# 7    p42_LPS_2_Apr22     ER    p42LPS       LPS         2 45296068 0.01
# 8     p30_UT_1_Apr22     ER     p30UT        UT         1 49725796 0.02
# 9    p42_LPS_1_Apr22     ER    p42LPS       LPS         1 58655447 0.02
# 10  p30_UT_Nov21_INP     ER  p30INPUT        UT         1 47004736 0.01
# 11  p42_UT_Nov21_INP     ER  p42INPUT        UT         1 44743006 0.01
# 12 p30_LPS_Apr22_INP     ER p30INPLPS       LPS         1 33762766 0.01
# 13 p42_LPS_Apr22_INP     ER p42INPLPS       LPS         1 47847282 0.01
# 14  p30_UT_Apr22_INP     ER  p30INPUT        UT         1 48003326 0.01
# 15  p42_UT_Apr22_INP     ER  p42INPUT        UT         1 46968976 0.01
saveRDS(Mdba.count,"Mdba.counts.ER.inclINP.RDS")

pdf(paste0("CEBPa_affinity_based_heatmap.inclINP.pdf"))
plot(Mdba.count)
dev.off()

pdf(paste0("CEBPa_affinity_based_PCA.inlcINP.pdf"))
dba.plotPCA(Mdba.count,  attributes=c(DBA_CONDITION), label=DBA_ID)
dev.off()
normCounts <- dba.peakset(Mdba.count, bRetrieve=TRUE, DataType=DBA_DATA_FRAME)
write.table(normCounts,"counts_incl_input.tsv",quote = F,row.names = F,sep = "\t")
head(normCounts)
colSums(normCounts[,c(4:ncol(normCounts))])
